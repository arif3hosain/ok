...
public class DashboardServiceImpl implements DashboardService {
     ...
     * When fewer than {@link #SHOWN} qualify, backfill with the next-soonest closing
     * tenders so the section still fills out to {@link #SHOWN} rows (matching the height
     * of "Best matches now") whenever enough closing-soon tenders exist at all.
     */
    static List<TenderSummaryResponse> worthActingOn(List<TenderSummaryResponse> closingSoon) {
        Comparator<TenderSummaryResponse> byClosingDate = Comparator.comparing(
                TenderSummaryResponse::closingAt, Comparator.nullsLast(Comparator.naturalOrder()));

        List<TenderSummaryResponse> priority = closingSoon.stream()
                .filter(t -> t.wishlisted() || t.grade() == MatchGrade.S || t.grade() == MatchGrade.A)
                .sorted(byClosingDate)
                .limit(SHOWN)
                .toList();

        if (priority.size() >= SHOWN) {
            return priority;
        }

        List<TenderSummaryResponse> filled = new java.util.ArrayList<>(priority);
        closingSoon.stream()
                .filter(t -> !priority.contains(t))
                .sorted(byClosingDate)
                .limit(SHOWN - priority.size())
                .forEach(filled::add);
        filled.sort(byClosingDate);
        return filled;
    }
